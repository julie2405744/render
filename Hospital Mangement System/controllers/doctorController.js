// Hayat Hospital — controllers/doctorController.js
// ---------------------------------------------------------
// @Author: Member 5 (Interactive Operations Developer)
// @Responsibility: Doctor dashboard, cascading events (shift update -> cut queue -> trigger emails)
// ---------------------------------------------------------
...
// ── Helper ────────────────────────────────────────────
// Parse a 12-hour time string like "12:28 PM" into total minutes from midnight.
...
// Calculate max patients for a time slot and avg consult time.
// timeSlot format: "May 19, 1:00 PM - 5:00 PM"
...
// POST /doctor/update-slots — doctor changes their available hours
exports.updateSlots = async (req, res) => {
    ...
    try {
        ...
        // Cancel overflow bookings — send capacity_reduced email to removed patients
        ...
        // Notify remaining patients of the new slot — send slots_updated email
        const remaining = await Booking.find({ doctorName: doctor.name });
        for (const b of remaining) {
            const patient = await User.findById(b.patientId);
            if (patient) {
                // Parse date and time out of timeSlot string (e.g. "May 19, 1:00 PM - 5:00 PM")
...
// POST /doctor/clear-slot — cancel ALL bookings and remove the time slot
exports.clearSlot = async (req, res) => {
    ...
    try {
        ...
        // Notify every booked patient — send appointment_canceled email
        ...
        // Clear the doctor's time slot and reset capacity
        doctor.timeSlot    = '';
